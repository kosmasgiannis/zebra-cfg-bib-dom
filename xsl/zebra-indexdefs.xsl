...
<xsl:stylesheet 
  ...
  <xsl:template match="marc:record">                
    ...
    <xsl:variable name="controlField005" 
                  select="normalize-space(marc:controlfield[@tag='005'])"/>
...
-->
      ...
      <xsl:call-template name="local_rules"/>
    </z:record>
  </xsl:template>
  ...
  <xsl:template name="local_rules">
     ...
     <xsl:for-each select="marc:datafield[@tag='852']/marc:subfield[@code='b']">
       <z:index name="Collection:w Collection:p Collection:s">
         <xsl:value-of select="./text()"/>
       </z:index>
     </xsl:for-each>
...
-->
  ...
  <xsl:template name="bib1_rules">
       ...
       <!-- att 63              Note -->
       <!-- att 1000            Author-title --> <!-- <xsl:call-template name="Author-title"/> -->
       ...
       <xsl:call-template name="date-added-todb"/>
       <!-- att 1012            Date/time-last-modified -->
       <xsl:call-template name="date-last-modified"/>
...
Date-publication       31  The date (usually year) in      008/07-10, 260$c
  ...
  <xsl:template name="Date-of-publication">
    <xsl:for-each select="marc:datafield[@tag='260']/marc:subfield[@code='c']">
      <z:index name="Date:w Date:p Date:s Date-of-publication:w Date-of-publication:p Date-of-publication:s any:w">
        <xsl:value-of select="./text()"/>
      </z:index>
    </xsl:for-each>
  </xsl:template>
...
publication                was published.
  ...
  <xsl:template name="Place-publication">
    <xsl:for-each select="marc:datafield[@tag='260']/marc:subfield[@code='a']">
      <z:index name="Place-publication:w Place-publication:p any:w">
        <xsl:value-of select="./text()"/>
      </z:index>
    </xsl:for-each>
  </xsl:template>
...
Name-publisher       1018  The organization responsible    260$b
  ...
  <xsl:template name="Publisher">
    <xsl:for-each select="marc:datafield[@tag='260']/marc:subfield[@code='b']">
      <z:index name="Publisher:w Publisher:p any:w">
        <xsl:value-of select="./text()"/>
      </z:index>
    </xsl:for-each>
  </xsl:template>
...
<!--
    <xsl:for-each select="marc:datafield[@tag='600']
      ...
      <z:index name="Subject-heading:w">
        <xsl:for-each select="marc:subfield[contains('abcdefghjklmnopqrstuvxyz',@code)]">
...
-->
    ...
    <xsl:for-each select="marc:datafield[@tag='830']">
      <z:index name="any:w title-series:w title-series:p title-series:s">
        <xsl:value-of select="marc:subfield[@code='a']/text()"/>
        <xsl:for-each select="marc:subfield[contains('dfgiklmnoprst',@code)]">
          <xsl:text> </xsl:text>
          <xsl:value-of select="./text()"/>
        </xsl:for-each>
      </z:index>
